...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        BPlusTree fromDisk = new BPlusTree(bufferManager, metadata, lockContext);
        LeafNode resideLeafNode = fromDisk.root.get(key);
        return resideLeafNode.getKey(key);
    }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        return new BPlusTreeIterator(root, -1);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
        return new BPlusTreeIterator(root.get(key), InnerNode.numLessThan(key, root.get(key).getKeys()) - 1);
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> newChildEntry = this.root.put(key, rid);
        ...
        if (newChildEntry.isPresent()) {
            ...
            InnerNode newRoot = new InnerNode(this.metadata, this.bufferManager, newKeys,
                    newChildren, this.lockContext);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        for (;;){
            if (data.hasNext()){
                Optional<Pair<DataBox, Long>> pairs = root.bulkLoad(data, fillFactor);
                if(pairs.isPresent()) {
                    DataBox first = pairs.get().getFirst();
                    List<DataBox> dataBoxes = new ArrayList<>();
                    List<Long> children = new ArrayList<>();
                    dataBoxes.add(first);
                    children.add(root.getPage().getPageNum());
                    children.add(pairs.get().getSecond());
                    updateRoot(new InnerNode(this.metadata, bufferManager, dataBoxes, children, lockContext));
                }
            }else {
                break;
            }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(BPlusNode node, int currentIndex) {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (this.currentIndex != this.node.getRids().size() - 1) {
                return true;
            } else {
        ...
        public RecordId next() {
            ...
            if (!hasNext()) {
                ...
                if (currentIndex != node.getRids().size() - 1) {
                    currentIndex += 1;
                } else {
                    node = node.getRightSibling().get();
                    currentIndex = 0;
                }
                return node.getRids().get(currentIndex);
            }
        }
    }
}
