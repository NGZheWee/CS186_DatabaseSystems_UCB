...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int position = numLessThanEqual(key,this.keys);
        BPlusNode nextNode = this.getChild(position);
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        // TODO(proj2): implement
        BPlusNode nextNode = this.getChild(0);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int position = numLessThanEqual(key,this.keys);
        BPlusNode nextNode = this.getChild(position);
        Optional<Pair<DataBox, Long>> newChildEntry = nextNode.put(key, rid);

        if (newChildEntry.isPresent()) {
            DataBox splitKey = newChildEntry.get().getFirst();
            Long newNodePageNum = newChildEntry.get().getSecond();

            int order = this.metadata.getOrder();
            this.keys.add(position, splitKey);
            this.children.add(position+1, newNodePageNum);

            if ((order * 2) >= this.keys.size()) {
                this.sync();
                ...
                InnerNode newInnerNode = new InnerNode(this.metadata, this.bufferManager, newKeys,
                        newChildren, this.treeContext);

                splitKey = this.keys.get(order);
                newNodePageNum = newInnerNode.page.getPageNum();

                while (keys.size() > order) {
                    keys.remove(keys.size() - 1);
                }
                while (children.size() > order+1) {
                    children.remove(children.size() - 1);
                }

                this.sync();
                newChildEntry = Optional.of(new Pair<>(splitKey, newNodePageNum));
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        for (;;){
            if (keySize < 2 * d){
                if (data.hasNext()) {
                    BPlusNode rightNode = BPlusNode.fromBytes(metadata, bufferManager, treeContext,
                            children.get(children.size() - 1));
                    Optional<Pair<DataBox, Long>> pairFromChild = rightNode.bulkLoad(data, fillFactor);
                    if (!pairFromChild.isPresent()) {
                        continue;
                    }else {
                        DataBox firstChild = pairFromChild.get().getFirst();
                        Long secondChild = pairFromChild.get().getSecond();
                        int idx = InnerNode.numLessThanEqual(firstChild, keys);
                        keySize += 1;
                        children.add(idx + 1, secondChild);
                        keys.add(idx, firstChild);
                    }
                }else {
                    break;
                }
        ...
        if (data.hasNext()) {
            for (;;){
                if ((!data.hasNext())){
                    ...
                    BPlusNode rightNode = BPlusNode.fromBytes(metadata, bufferManager, treeContext,
                            children.get(children.size() - 1));
                    Optional<Pair<DataBox, Long>> pairs = rightNode.bulkLoad(data, fillFactor);
                    if (!pairs.isPresent()) {
                        continue;
                    }else {
                        DataBox firstChild = pairs.get().getFirst();
                        Long secondChild = pairs.get().getSecond();
                        int idx = InnerNode.numLessThanEqual(firstChild, keys);
                        children.add(idx + 1, secondChild);
                        keys.add(idx, firstChild);
                        DataBox spiltKey = keys.get(d);
                        ...
                        InnerNode newInner = new InnerNode(metadata, bufferManager, dataBoxes, newChildren, treeContext);
                        sync();
                        return Optional.of(new Pair<>(spiltKey, newInner.getPage().getPageNum()));
                    }
                }
            }
        }
    ...
    public void remove(DataBox key) {
        // TODO(proj2): implement
        LeafNode resideNode = this.get(key);
        resideNode.remove(key);
    }
