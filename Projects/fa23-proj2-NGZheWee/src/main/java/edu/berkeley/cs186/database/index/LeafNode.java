...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int existIndex = this.keys.indexOf(key);
        if (existIndex != -1) {
            throw new BPlusTreeException("No Duplicate Entries Allowed!");
        }

        int order = this.metadata.getOrder();

        int position = InnerNode.numLessThanEqual(key, this.keys);
        this.keys.add(position, key);
        this.rids.add(position, rid);
        ...
        if ((order * 2) >= this.keys.size()) {
            ...
            LeafNode newLeafNode = new LeafNode(metadata, bufferManager, newKeys,
                    newRids, this.rightSibling, treeContext);

            for (int i = order; i < keys.size(); i++) {
                newLeafNode.keys.add(this.keys.get(i));
                newLeafNode.rids.add(this.rids.get(i));
                newLeafNode.sync();
            }
            while (keys.size() > order) {
                keys.remove(keys.size() - 1);
            }
            while (rids.size() > order) {
            ...
            DataBox splitKey = newLeafNode.keys.get(0);
            long newNodePageNum = newLeafNode.page.getPageNum();
            Optional<Pair<DataBox, Long>> newChildEntry = Optional.of(new Pair<>(splitKey, newNodePageNum));
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int maxSize = (int) Math.ceil(fillFactor * 2 * metadata.getOrder());
        for (;;){
            if (keyNums >= maxSize || !data.hasNext()){
                break;
            }
            ...
            int keyIndex = InnerNode.numLessThanEqual(dataBox, keys);
            rids.add(keyIndex, recordId);
            keys.add(keyIndex, dataBox);
            keyNums += 1;
        }
        if (data.hasNext()) {
            Pair<DataBox, RecordId> dataBoxRecordIdPair = data.next();
            DataBox key = dataBoxRecordIdPair.getFirst();
            RecordId recordId = dataBoxRecordIdPair.getSecond();

            int idx = InnerNode.numLessThanEqual(key, keys);
            rids.add(idx, recordId);
            keys.add(idx, key);

            List<RecordId> newRecords = rids.subList(maxSize, rids.size());
            rids = rids.subList(0, maxSize);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        assert (1 == pageBuffer.get());
        long tmp = pageBuffer.getLong();
        ...
        int num = pageBuffer.getInt();
        for (int i = 0; i < num; i++) {
            dataBoxes.add(DataBox.fromBytes(pageBuffer, metadata.getKeySchema()));
            recordIds.add(RecordId.fromBytes(pageBuffer));
        }
        return new LeafNode(metadata, bufferManager, page, dataBoxes, recordIds, rightSibling, treeContext);
    }
