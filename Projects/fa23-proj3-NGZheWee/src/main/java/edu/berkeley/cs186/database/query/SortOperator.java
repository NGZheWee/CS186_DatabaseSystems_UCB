...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        ArrayList<Iterator<Record>> recordIters = new ArrayList<>();
        for (int i = 0; i < runs.size(); i++) {
            Iterator<Record> itor = runs.get(i).iterator();
            queue.offer(new Pair<>(itor.next(), i));
            recordIters.add(itor);
        }
        Run res = new Run(transaction, getSchema());
        for (;;){
            if (queue.isEmpty()) {
            ...
            Pair<Record, Integer> recInt = queue.poll();
            res.add(recInt.getFirst());
            Iterator<Record> itor = recordIters.get(recInt.getSecond());
            if (!itor.hasNext()) {
                continue;
            } else {
                queue.offer(new Pair<>(itor.next(), recInt.getSecond()));
            }
        }
        return res;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        // select numBuffers - 1 runs to merge every time
        ArrayList<Run> res = new ArrayList<>();
        for (int i = 0; i < runs.size(); i += numBuffers - 1) {
            ArrayList<Run> tmp = new ArrayList<>();
    ...
    public Run sort() {
        ...
        for (;;) {
            if (sourceIterator.hasNext()) {
                Iterator<Record> recItor = QueryOperator.getBlockIterator(sourceIterator, getSchema(), numBuffers);
                Run tmpRecs = sortRun(recItor);
                res.add(tmpRecs);
            } else {
                break;
            }
