...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        records.forEach(record -> {
            ...
            int partitionNum = HashFunc.hashDataBox(columnValue, pass) % partitions.length;
            if (partitionNum < 0) {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        buildRecords.forEach(buildRecord -> {
            DataBox key = buildRecord.getValue(buildColumnIndex);
            List<Record> value = hashTable.get(key) == null ? new ArrayList<>() : hashTable.get(key);
            value.add(buildRecord);
            hashTable.put(key, value);
        });

        // probing
        probeRecords.forEach(probeRecord -> {
            DataBox key = probeRecord.getValue(probeColumnIndex);
            if (hashTable.containsKey(key)) {
                if (!probeFirst) {
                    hashTable.get(key).forEach(record -> {
                        joinedRecords.add(record.concat(probeRecord));
                    });
                } else {
                    hashTable.get(key).forEach(record -> {
                        joinedRecords.add(probeRecord.concat(record));
                    });
                }
            }
        });
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        for (;;) {
            ...
            IntDataBox intDataBox = new IntDataBox(nextInt);
            int index = ((HashFunc.hashDataBox(intDataBox, 1) % 5) + 5) % 5;
            Record record = createRecord(nextInt);
