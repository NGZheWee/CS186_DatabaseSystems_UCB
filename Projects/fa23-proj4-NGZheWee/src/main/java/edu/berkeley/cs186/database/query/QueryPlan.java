...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        final int[] minCost = {sequentialScanCost};
        final QueryOperator[] minOp = {sequentialScan};
        final int[] except = {-1};
        eligibleIndexColumns.forEach(x -> {
            SelectPredicate selectPredicate = selectPredicates.get(x);
            ...
            QueryOperator indexScan = new IndexScanOperator(transaction, table, columnName, predicate, value);
        ...
        minOp[0] = addEligibleSelections(minOp[0], except[0]);
        return minOp[0];
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        prevMap.keySet().forEach(tableSet -> {
            joinPredicates.forEach(join -> {
                if (tableSet.contains(join.rightTable) && !tableSet.contains((join.leftTable))) {
                    Set<String> tmpSet = new HashSet<>();
                    tmpSet.add(join.leftTable);
                    QueryOperator joinOp = minCostJoinType(pass1Map.get(tmpSet), prevMap.get(tableSet), join.leftColumn, join.rightColumn);
                    Set<String> tmp = new HashSet<>(tableSet);
                    tmp.add(join.leftTable);
                    result.put(tmp, joinOp);
                } else if (tableSet.contains(join.leftTable) && !tableSet.contains(join.rightTable)) {
                    Set<String> tmpSet = new HashSet<>();
                    tmpSet.add(join.rightTable);
                    QueryOperator joinOp = minCostJoinType(prevMap.get(tableSet), pass1Map.get(tmpSet), join.leftColumn, join.rightColumn);
                    Set<String> tmp = new HashSet<>(tableSet);
                    tmp.add(join.rightTable);
                    result.put(tmp, joinOp);
                }
            });
        });
    ...
    public Iterator<Record> execute() {
        ...
        tableNames.forEach(tableName -> {
            ...
            pass1Map.put(new HashSet<String>(){{add(tableName);}}, minSingleOp);
        });
        Map<Set<String>, QueryOperator>[] arr = new HashMap[tableNames.size() + 1];
        arr[0] = null;
        arr[1] = pass1Map;
        ...
        finalOperator = minCostOperator(arr[tableNames.size()]);
        this.addGroupBy();
        this.addProject();
        this.addSort();
        this.addLimit();
